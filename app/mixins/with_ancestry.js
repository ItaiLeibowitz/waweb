	...
	ancestorsArray: function(){
		...
		if (ancestry && ancestry.length > 0) {
			...
			response = ancestryArray.map(function(el, i){
				return {name: ancestryNamesArray[i], slug: "%@-%@".fmt(el,ancestryNamesArray[i]),offsetClass: 'offset-' + i}
			})
		}
		response.push({name: this.get('name'), slug: this.get('slug'), offsetClass:'offset-' + response.length});
		return response;
